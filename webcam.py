...
## Algoritms:computer::
- Detetcing Faces:
    - Faces in each frame of webcam feed is identified using Google MediaPipe library face Detetction alogorithm .This library includes many other computer vision models such as object detection,3D - model bounding,Object Tracking etc.

- Detetcing Masks:

    - For labeling the faces if it is with mask-on or not , A Deep Convolution network is used.This network is implemented using transfer learning of MobileNetv2.This network takes a 224 by 224 by 3 size image as input is passed through various layers of convolutions and pooling to extract hierarchical features from image.
    ...
    while checkbox:
        ...
        with mp_face_detection.FaceDetection(model_selection=model_selection, min_detection_confidence=model_detect_conf) as face_detection:
            ...
            #       continue
                ...
                server.login("ban676fall2022@gmail.com", "ttczergdnflqcfgm")

                if out[-1] ==0:
                    st.markdown("Trigger Email")
                    message = 'Subject: {}\n\n{}'.format('Mask Detection', 'Administrator!! No Mask Detected')
                    server.sendmail("ban676fall2022@gmail.com", "backupmacbookair2020@gmail.com",message)
